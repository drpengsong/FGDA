...
class mySolver(object):
    ...
    def __init__(self, config):
        ...
        self.source_loader = get_gsv_dataloader(case='GSV', batch_size=self.BATCH_SIZE[0])
        print('source : {}_{}'.format(len(self.source_loader), len(self.source_loader.dataset)))
        self.target_loader = get_gsv_dataloader(case='Web', batch_size=self.BATCH_SIZE[1])
        print('target : {}_{}'.format(len(self.target_loader), len(self.target_loader.dataset)))

        self.source_loader_test = self.source_loader
        self.target_loader_test = get_gsv_dataloader(case='Webtest', batch_size=self.BATCH_SIZE[1])
    ...
    def build_model(self):
        self.featureExactor = ResNet()
        self.featureExactor.cuda()

        self.featureExactor1 = ResNet()
        ...
        self.domain = [1, 1]
        for i in range(2):
            self.domain[i] = nn.DataParallel(DomainDis().cuda())
    ...
    def train_perEpoch(self, epoch):
        ...
        for batch_idx in range(train_steps):
            ...
            f_loss = classification_loss + self.config['domain'] * domaindisloss + self.config['class'] * class_invariant
            f_loss.backward()
    ...
    def train(self):
        ...
        self.best_featureExactor = self.featureExactor
        self.best_featureExactor1 = self.featureExactor1
        self.best_classfier = self.classfier
        ...
        for e in tqdm(range(0, self.EPOCHS)):
            ...
            test_target = self.test(self.target_loader_test, e)
            ...
            if test_target['accuracy'] > max_acc:
                max_acc = test_target['accuracy']
                i = e
                self.best_featureExactor = self.featureExactor
                self.best_featureExactor1 = self.featureExactor1
                self.best_classfier = self.classfier
        ...
        root = '/data/result/FGDA/FGDA/semi-supervised/GSV(ResNet)-{:.4f}'.format(max_acc) + '/'
        if not os.path.exists(root):
            os.makedirs(root)

        tqdm.write(utils.save_net(self.best_classfier, root + 'classifier_final.tar'))
        tqdm.write(utils.save_net(self.best_featureExactor, root + 'featureExactor_final.tar'))
        tqdm.write(utils.save_net(self.best_featureExactor1, root + 'featureExactor1_final.tar'))
        tqdm.write(utils.save_net(self.best_domain0, root + 'domainDiscriminator0_final.tar'))
        tqdm.write(utils.save_net(self.best_domain1, root + 'domainDiscriminator1_final.tar'))
